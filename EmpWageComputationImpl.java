...
public class EmpWageComputationImpl implements EmpWageInterface {
	private static final int IS_EMP_FULL_TIME = 0;
	...
	public int getMonthlyWage(Company company) {
		...
		while (totalEmpHours < company.maximunWorkHours && totalWorkingDays < company.workingDaysInMonth) {
			...
			empHours = getWorkingHours(empCheck);// call method with passing argument and store value in instace var
													// empHours
	...
	// method for calculate the result and get the output
	public void getCalculationDailyWage() {
		for (int i = 0; i < numOfCompany; i++) {
			...
			System.out.println(
					"Company: " + companyArray.get(i).company + " Employee Wage :" + companyArray.get(i).totalSalary);
			int oldWage = 0;
			System.out.println("Day\t\t\tWage\t\t\tTotal Wage");
			for (int j = 0; j < companyArray.get(i).dailyWage.size(); j++) {
				System.out.println("Day: " + j + "\t\t" + (companyArray.get(i).dailyWage.get(j) - oldWage) + "\t\t\t\t"
						+ companyArray.get(i).dailyWage.get(j));
